...
export default class ReplyMessages {
    ...
    private async lineReply(text: string, msg: Message, stickerId: string | null = null): Promise<void> {
        try {
            await axios.post(`https://discord.com/api/v9/channels/${msg.channel.id}/messages`, {
                content: text,
                tts: false,
                sticker_ids: stickerId ? [stickerId] : null,
                message_reference: {
                    message_id: msg.id,
    ...
    private async replyMsg(text: string, msg: Message, interaction: boolean): Promise<void> {
        if (interaction) {
            ...
            if (!text) {
                ...
                this.sendInteraction(msg, lucky(50));
        ...
        await sleep(100 * text.length + 1000);
    ...
    private async workSimsimi(content: string): Promise<string | null> {
        try {
            ...
            if (this.hasBannedWord(content) || this.hasBannedWord(res)) return '<banned>';
